    ...
    const ProjectsData = (conditional,source = undefined,start) => {
       ...
       let Data = useContext(ProjectsContext).map(
           ...
           (project) => {
                ...
                {
                    return (
                        <div title={ReturnTitle(+project.id)} key={project.id} style={{ margin: "50px auto", borderRadius: "25px", background: Modes[0], width: "18em", height: "20em"}}>
                                    <p>{project.name}</p>
...
function ProjectsList({type,icon,source = "my"}) {
    ...
    return (<>
        ...
        <ProjectsList type="learning video" source="tarmez academy" icon="tarmez academy.png"/>
    </>);
}
